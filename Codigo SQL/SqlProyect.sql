...
CREATE TABLE IF NOT EXISTS `SpaceAdmind`.`PrivacidadUsuario` (
  `idPrivacidadUsuario` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `SpaceAdmind`.`Modificacion` (
  ...
  INDEX `ModEspacio_idx` (`ModEspacio` ASC),
  ...
  CONSTRAINT `ModNivel`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `ModEspacio`
    FOREIGN KEY (`ModEspacio`)
    REFERENCES `SpaceAdmind`.`Espacio` (`idEspacioCompuesto`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `SpaceAdmind`.`HorarioSede` (
  ...
  PRIMARY KEY (`idHorarioSede`, `idDiasHabiles`, `idOrganizacion`),
  INDEX `IdDiasHabilesSede_idx` (`idDiasHabiles` ASC),
  INDEX `idOrgSedeHorario_idx` (`idOrganizacion` ASC),
  CONSTRAINT `IdDiasHabilesSede`
    FOREIGN KEY (`idDiasHabiles`)
    REFERENCES `SpaceAdmind`.`DiasHabiles` (`idDiasHabiles`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `idOrgSedeHorario`
    FOREIGN KEY (`idOrganizacion`)
    REFERENCES `SpaceAdmind`.`Organizacion` (`idOrganizacion`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
